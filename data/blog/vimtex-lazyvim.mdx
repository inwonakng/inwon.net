...
Custom snippets is one of the power powerful tools you can leverage when writing your latex with vim and vimtex.
...
Note that I am using the relative path. 
Since everything under `lua` is loaded by nvim, this ends up pointing to `~/config/nvim/snippets`.

Inside that directory, you can create a snippets file that uses vscode style snippets.
In order to do this, you need to have a `package.json` file at the root of the snippets directory.
You can also use subdirectories to maintain snippets for different purposes.
